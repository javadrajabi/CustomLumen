...
namespace App\Libraries\Sanitizer\Laravel;

use Closure;
use InvalidArgumentException;
use App\Libraries\Sanitizer\Contracts\Filter;
use App\Libraries\Sanitizer\Sanitizer;
...
class Factory
{
    ...
    public function extend($name, $customFilter)
    {
        ...
        if (!($customFilter instanceof Closure) && !in_array(Filter::class, class_implements($customFilter))) {
            throw new InvalidArgumentException('Custom filter must be a Closure or a class implementing the App\Libraries\Sanitizer\Contracts\Filter interface.');
        }
